void modifCont() {
    ...
    float sc=scrollbarModif.decateori*scrollbarModif.h4;
   	//text("modif",200,200);
   	if (/*parola.s=="" || */numeUtilizator.y!=height/8+R*4 || parola.y!=height/8+R*13) {
    ...
    //if()
   	if (EROAREINREGISTRARE) {
        ...
        text("Asigura-te ca ai completat toate campurile corect!\nEmailul/Numele de utilizator poate sunt deja folosite",width/7,email.y+email.h+R*3-sc,width,R*4);
    }
    fill(-1);
    textSize(R*2);
