int getTestIdByName(String testName, int profID){
	int id = -1;
	if(db.connect()){
		db.query("SELECT testID, testName, profID FROM testdata WHERE testName='"+testName+"' AND profID="+profID);
		if(db.next())
			id = db.getInt("testID");
	}
...
int getTemaIdByName(String temaName, int profID){
	int id = -1;
	if(db.connect()){
		db.query("SELECT temaID, temaName, profID FROM temadata WHERE temaName='"+temaName+"' AND profID="+profID);
		if(db.next())
			id = db.getInt("temaID");
	}
