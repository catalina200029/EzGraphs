		...
		if (isBetween(x,x+w,mouseX) && isBetween(y,y+h,mouseY)) {
			...
			if (mousePressed) {
				...
				if (START==false)
					...
					if (numeUtilizator.s!="" && parola.s!="") {
						...
						if(!OKLOGIN){
							...
							if(clsUsr>0){
								...
								menu2.tabs[0].sub[3] = new SubMenu("Feeback", 24, menu2.tabs[0].seekPos, menu2.tabs[0].seekCol, menu2.tabs[0].seekSize, menu2.tabs[0].animSpd, menu2.tabs[0].seekW, 2*menu2.tabs[0].h/3);
								...
								menu2.tabs[0].sub[0] = new SubMenu(usrrr, 8, menu2.tabs[0].seekPos, menu2.tabs[0].seekCol, menu2.tabs[0].seekSize, menu2.tabs[0].animSpd, menu2.tabs[0].seekW, 2*menu2.tabs[0].h/3);
								menu2.tabs[0].sub[1] = new SubMenu("Feeback", 24, menu2.tabs[0].seekPos, menu2.tabs[0].seekCol, menu2.tabs[0].seekSize, menu2.tabs[0].animSpd, menu2.tabs[0].seekW, 2*menu2.tabs[0].h/3);
							...
							if(!usrProf){
								...
								menu2.tabs[2].sub[3] = new SubMenu("Atribuire Teme/Teste", 21, menu2.tabs[2].seekPos, menu2.tabs[2].seekCol, menu2.tabs[2].seekSize, menu2.tabs[2].animSpd, menu2.tabs[2].seekW, 2*menu2.tabs[2].h/3);
								menu2.tabs[2].sub[4] = new SubMenu("Jocuri", 3, menu2.tabs[2].seekPos, menu2.tabs[2].seekCol, menu2.tabs[2].seekSize, menu2.tabs[2].animSpd, menu2.tabs[2].seekW, 2*menu2.tabs[2].h/3);
							}
						}
					}
					...
					if (nume.s!="" && prenume.s!="" && numeUtilizator.s!="" && parola.s!="" &&  parola2.s!="" && email.s!="" && compareStrings(parola.s,parola2.s) && statut.selectat!=-1) {
						int isProf;
					...
					if (compareStrings(s, "Note")) currentModule=9;
					...
					if(nume.s!="" && prenume.s!="" && numeUtilizator.s!="" && parola.s!="" &&  parola2.s!="" && email.s!="" && compareStrings(parola.s,parola2.s) && statut.selectat!=-1){
						...
						while(db.next()){
							String tempName = db.getString("nickname");
							String tempEmail = db.getString("email");
							if( (compareStrings(tempName, numeUtilizator.s) && !compareStrings(tempName, nicknmUsr)) || (compareStrings(tempEmail, email.s) && !compareStrings(tempEmail, emailUsr)))
								EROAREINREGISTRARE = true;
						}
						if(!EROAREINREGISTRARE){
							db.execute("UPDATE users SET name='"+nume.s+"', surname='"+prenume.s+"', nickname='"+numeUtilizator.s+"', email='"+email.s+"', pass='"+parola.s+"', isProfessor="+isProf+" WHERE userID="+IDUSER+"");
