			...
			for (i=4;i>=0;i--)
				if (isBetween(x+i*w*3/2,x+i*w*3/2+w,mouseX) && isBetween(y,y+h,mouseY)) {
					...
					if (mousePressed && delay==0) {
						...
						if(db.connect()){
							db.query("SELECT * FROM lectiedata WHERE lectieName='"+lectieCurentaprof.s2+"' AND lectieData='"+lectieCurentaprof.s3+"'");
							if(db.next()){
								int scr = db.getInt("scor");
								int nrVotes = db.getInt("nrVotes") + 1;
								int lectieID = db.getInt("lectiID");
								db.execute("UPDATE lectiedata SET scor="+((lectieCurentaprof.vs.val * (nrVotes-1) + scr) / nrVotes)+" WHERE lectieID="+lectieID);
							}
						}
