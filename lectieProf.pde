	...
	void display() {
		...
		if (isBetween(x+w2,x+w2+w-w2*2,mouseX) && isBetween(y+w2*7,y+w2*7+h-w2*8,mouseY)) {
			if (mousePressed && delay==0) {
				delay=DELAY;
				if (currentModule==26) {
					lectieCurentaprof=new lectieProf(R*3,R*9,w,lungimeLectie(s),s,s2,s3,0,1);
					scrollbarLectie=new scrollbar2(width-R/2,R*2,R/2,height-R*2,int((lectieCurentaprof.y+lectieCurentaprof.h+R*3)/4/R/3+1));
					currentModule=27;
				}
			}
			fill(-1,90);
