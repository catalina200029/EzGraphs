	...
	Form(PVector pos, color c, float w, float h, float anim, int i){
		...
		switch(index){
			case 1:{	//Nume + Tip Lectie
				tipLectie = new MultipleSelectionDropDown(new PVector(pos.x, pos.y), c6, c6_o, 30, (width - 20)/2, height/10, 0.2);
				tipLectie.addMainTab("Tip Lectie", 1);
				tipLectie.addMainTab("Vizibilitate", 2);
				...
				tipLectie.mainTabs.get(1).addSubTab("Clasa mea", 1);
				tipLectie.mainTabs.get(1).addSubTab("Public", 2);
			...
			case 2:{	//Lectie + Graf Anim
				...
				/*if(tipLectie.mainTabs!=null && tipLectie.mainTabs.get(0).indexSelected==1)
					grafLectie.oriented = false;
				else grafLectie.oriented = true;*/
			...
			if(SENT)return;
		   	if(db.connect()){
		   		...
		   		if(db.next()){
		   			...
		   			//faCodGraf()
					db.execute("INSERT INTO lectiedata VALUES(NULL, "+profID+", '"+currForm.forms.get(1).txtb.s+"', '"+currForm.forms.get(0).numeLectie.s+"', 0, 0)");
		   			SENT=true;
		   		}
